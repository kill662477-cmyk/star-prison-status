...
try{
  const page=await browser.newPage({viewport:{width:540,height:900},deviceScaleFactor:1});
  await page.goto('http://127.0.0.1:4178/?capture=1&cols=2');
  ...
  await page.evaluate(async()=>{await document.fonts.ready;await Promise.all(['assets/mugshots-approved.png','assets/prison-wall.png'].map(src=>new Promise((resolve,reject)=>{const img=new Image();img.onload=resolve;img.onerror=reject;img.src=src;})));});
  ...
  if(geometry.columns!==2)throw Error('Capture must have two columns');
